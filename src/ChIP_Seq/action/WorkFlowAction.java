...
import ChIP_Seq.*;
import ChIP_Seq.bean.*;
import ChIP_Seq.execute.BaseExecute;
...
public class WorkFlowAction extends ActionSupport{
    ...
    private String Scope_demo,peakcalling_visual_scope,mapping_visual_scope;
    ...
    public String customWorkflow() throws Exception {
     if(workflow_type==1 && workflowCustom_reference.equals("Escherichia_coli_K12.fasta") && workflowCustom_sequence.equals("SRR576933.fastq") && workflowCustom_control.equals("SRR576938.fastq")){
         ...
         //写入XML文件
         File file = new File(bampath_treatment);
         String name = file.getName();
         File file1 = new File(bampath_control);
         String name1 = file1.getName();
         File file2 = new File(workflowCustom_reference);
         String name2 = file2.getName();
         String JobID=xmlUtil.xmlAdd_Jobs_Demo(name + " ; " + name1 + " ; " + name2, "WorkFlow Demo dataset", "error", "email",time);
         ...
         String remote_sequence, remote_pair1, remote_pair2, remote_re, remote_control;
         List<String> Upload_path = uploadUtil.upload(fileField_workflowFileName, fileField_workflow);
         if (workflow_type == 1) {
             remote_sequence = Upload_path.get(1);
             remote_pair1 = "";
             remote_pair2 = "";
             ...
             remote_control = Upload_path.get(3);
             remote_re = Upload_path.get(0);
         }
         ////////////////////////////////////////////////////////////////////////////////////////
         ...
         String original;
         if (workflow_type == 1) {
             File temp = new File(remote_sequence);
             original = temp.getName().substring(0, temp.getName().length() - 6);
         } else {
             File temp = new File(remote_pair1);
             original = temp.getName().substring(0, temp.getName().length() - 6);
         }
         File temp1 = new File(remote_control);
         String control = temp1.getName().substring(0, temp1.getName().length() - 6);
         File temp = new File(remote_sequence);
         String out = temp.getName().substring(0, temp.getName().length() - 6);
         sampath = workflowCustom_path + "/" + original + ".sam";
         bampath_treatment = workflowCustom_path + "/" + original + ".sam";
         bampath_control = workflowCustom_path + "/" + control + ".sam";
         if (workflowCustom_peak == 1 || workflowCustom_peak == 2 || workflowCustom_peak == 5 || workflowCustom_peak == 7 || workflowCustom_peak == 11 || workflowCustom_peak == 12) {
             peakpath = workflowCustom_path + "/" + out + "_peaks.bed";
         } else if (workflowCustom_peak == 4) {
             peakpath = workflowCustom_path + "/" + out + "_summit.bed";
         } else if (workflowCustom_peak == 3) {
             peakpath = workflowCustom_path + "/" + out + "_narrowPeak.txt";
         } else {
         }
         ...
         appendMethodB(workflowCustom_path + "/workflow_input_temp.txt", bampath_treatment + "\r\n");
         appendMethodB(workflowCustom_path + "/workflow_input_temp.txt", bampath_control + "\r\n");
         appendMethodB(workflowCustom_path + "/workflow_input_temp.txt", peakpath + "\r\n");
         appendMethodB(workflowCustom_path + "/workflow_input_temp.txt", workflowCustom_path + "\r\n");
         appendMethodB(workflowCustom_path + "/workflow_input_temp.txt", remote_re + "\r\n");



         if (workflowCustom_mapping == 1) {
             BWAction bwAction = new BWAction();
             bwAction.setBWA_type(workflow_type);
             bwAction.setInput_BWA_reads_single(remote_sequence);
             bwAction.setInput_BWA_reads_pair1(remote_pair1);
             bwAction.setInput_BWA_reads_pair2(remote_pair2);
             bwAction.setRefer_index_BWA(refer_index_workCustom);
             bwAction.setInput_BWA_reference(remote_re);
             bwAction.setBuilt_in_BWA(workflowCustom_build);
             bwAction.setLong_short(long_short);
             bwAction.setBuildOrnot(1);
             bwAction.setAnalysisMode(1);
             bwAction.setInput_BWA_path(workflowCustom_path);
             bwAction.BWA_noUpload();
             stdoutlist_mapping1 = bwAction.getStdoutlist();
             ////////////////////////////////////////////////////////////////////////////////////////
             BWAction bwAction1 = new BWAction();
             bwAction1.setBWA_type(workflow_type);
             bwAction1.setInput_BWA_reads_single(remote_control);
             bwAction1.setInput_BWA_reference(remote_re);
             bwAction1.setRefer_index_BWA(refer_index_workCustom);
             bwAction1.setBuilt_in_BWA(workflowCustom_build);
             bwAction1.setLong_short(long_short);
             bwAction1.setBuildOrnot(2);
             bwAction1.setAnalysisMode(1);
             bwAction1.setInput_BWA_path(workflowCustom_path);
             bwAction1.BWA_noUpload();
             stdoutlist_mapping2 = bwAction1.getStdoutlist();
         }
         /************************************************************************************************/
         else if (workflowCustom_mapping == 2) {
             BowtieAction bowtieAction = new BowtieAction();
             bowtieAction.setType(workflow_type);
             bowtieAction.setInput_reads_bowtie(remote_sequence);
             bowtieAction.setInput_reads_pair1_bowtie(remote_pair1);
             bowtieAction.setInput_reads_pair2_bowtie(remote_pair2);
             bowtieAction.setRefer_index_bowtie(refer_index_workCustom);
             bowtieAction.setInput_re_bowtie(remote_re);
             bowtieAction.setBowtie_option_radio(1);
             bowtieAction.setInputOption_bowtie_mode(1);
             bowtieAction.setBowtie_mode_n("2");
             bowtieAction.setBowtie_mode_l("5");
             bowtieAction.setBowtie_mode_e("40");
             bowtieAction.setBowtie_maximun("125");
             bowtieAction.setBowtie_s("0");
             bowtieAction.setBowtie_u("100000000");
             bowtieAction.setBowtie_5("0");
             bowtieAction.setBowtie_3("0");
             bowtieAction.setBowtie_I("0");
             bowtieAction.setBowtie_nomaqround(1);
             bowtieAction.setBowtie_X("250");
             bowtieAction.setBowtie_a(1);
             bowtieAction.setBowtie_k("1");
             bowtieAction.setInput_path_bowtie(workflowCustom_path);
             bowtieAction.Bowtie_noUpload();
             stdoutlist_mapping1 = bowtieAction.getStdoutlist();
             ///////////////////////////////////////////////////////////////////////////////////////////////////
             BowtieAction bowtieAction1 = new BowtieAction();
             bowtieAction1.setType(workflow_type);
             bowtieAction1.setInput_reads_bowtie(remote_control);
             bowtieAction1.setRefer_index_bowtie(refer_index_workCustom);
             bowtieAction1.setInput_re_bowtie(remote_re);
             bowtieAction1.setBowtie_option_radio(1);
             bowtieAction1.setInputOption_bowtie_mode(1);
             bowtieAction1.setBowtie_mode_n("2");
             bowtieAction1.setBowtie_mode_l("5");
             bowtieAction1.setBowtie_mode_e("40");
             bowtieAction1.setBowtie_maximun("125");
             bowtieAction1.setBowtie_s("0");
             bowtieAction1.setBowtie_u("100000000");
             bowtieAction1.setBowtie_5("0");
             bowtieAction1.setBowtie_3("0");
             bowtieAction1.setBowtie_I("0");
             bowtieAction1.setBowtie_nomaqround(1);
             bowtieAction1.setBowtie_X("250");
             bowtieAction1.setBowtie_a(1);
             bowtieAction1.setBowtie_k("1");
             bowtieAction1.setInput_path_bowtie(workflowCustom_path);
             bowtieAction1.Bowtie_noUpload();
             stdoutlist_mapping2 = bowtieAction1.getStdoutlist();
         }
         /*************************************************************************************************/
         else if (workflowCustom_mapping == 3) {
             Bowtie2Action bowtie2Action = new Bowtie2Action();             //original fastq file
             bowtie2Action.setRefer_index(refer_index_workCustom);
             bowtie2Action.setType(workflow_type);
             bowtie2Action.setInput_reads(remote_re);
             bowtie2Action.setInput_reads_pair1(remote_pair1);
             bowtie2Action.setInput_reads_pair2(remote_pair2);
             bowtie2Action.setInput_re(remote_re);
             System.out.println("option:" + workflowCustom_option_bowtie2);
             bowtie2Action.setInputOption(workflowCustom_option_bowtie2);
             bowtie2Action.setInput_path(workflowCustom_path);
             bowtie2Action.Bowtie2_noUpload();
             stdoutlist_mapping1 = bowtie2Action.getStdoutlist();
             ///////////////////////////////////////////////////////////////////////////////////////////////////////
             Bowtie2Action bowtie2Action1 = new Bowtie2Action();                          //control fastq file
             bowtie2Action1.setType(workflow_type);
             bowtie2Action1.setRefer_index(refer_index_workCustom);
             bowtie2Action1.setInput_reads(remote_sequence);
             bowtie2Action1.setInput_re(remote_re);
             bowtie2Action1.setInputOption(workflowCustom_option_bowtie2);
             bowtie2Action1.setInput_path(workflowCustom_path);
             bowtie2Action1.Bowtie2_noUpload();
             stdoutlist_mapping2 = bowtie2Action1.getStdoutlist();
         }
         /************************************************************************************************/
         else if (workflowCustom_mapping == 4) {
             SOAPAction soapAction = new SOAPAction();
             soapAction.setRefer_index_SOAP(refer_index_workCustom);
             soapAction.setSOAP_type(workflow_type);
             soapAction.setInput_SOAP_reads_single(remote_sequence);
             soapAction.setInput_SOAP_reads_pair1(remote_pair1);
             soapAction.setInput_SOAP_reads_pair2(remote_pair2);
             soapAction.setInput_SOAP_reference(remote_re);
             System.out.println("option:" + workflowCustom_option_SOAP);
             soapAction.setOptionalParameters_SOAP(workflowCustom_option_SOAP);
             soapAction.setInput_SOAP_path(workflowCustom_path);
             soapAction.SOAP_noUpload();
             stdoutlist_mapping1 = soapAction.getStdoutlist();
             ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
             SOAPAction soapAction1 = new SOAPAction();
             soapAction1.setSOAP_type(workflow_type);
             soapAction1.setRefer_index_SOAP(refer_index_workCustom);
             soapAction1.setInput_SOAP_reads_single(remote_sequence);
             soapAction1.setInput_SOAP_reference(remote_re);
             soapAction1.setOptionalParameters_SOAP(workflowCustom_option_SOAP);
             soapAction1.setInput_SOAP_path(workflowCustom_path);
             soapAction1.SOAP_noUpload();
             stdoutlist_mapping2 = soapAction1.getStdoutlist();
         }
         /************************************************************************************************/
         else if (workflowCustom_mapping == 5) {
             BlastAction blastAction = new BlastAction();
             blastAction.setRefer_index_blast(refer_index_workCustom);
             blastAction.setBlast_type(2);
             blastAction.setInput_blast_reads(remote_sequence);
             blastAction.setInput_blast_reference(remote_re);
             blastAction.setOptionalParameters_blast(workflowCustom_option_BLAST);
             blastAction.setInput_blast_path(workflowCustom_path);
             blastAction.Blast_noUpload();
             stdoutlist_mapping1 = blastAction.getStdoutlist();
             ///////////////////////////////////////////////////////////////////////////////////////////////////////
             BlastAction blastAction1 = new BlastAction();
             blastAction1.setBlast_type(2);
             blastAction1.setRefer_index_blast(refer_index_workCustom);
             blastAction1.setInput_blast_reads(remote_control);
             blastAction1.setInput_blast_reference(remote_re);
             blastAction1.setOptionalParameters_blast(workflowCustom_option_BLAST);
             blastAction1.setInput_blast_path(workflowCustom_path);
             blastAction1.Blast_noUpload();
             stdoutlist_mapping2 = blastAction1.getStdoutlist();
         }
         /************************************************************************************************/
         else if (workflowCustom_mapping == 6) {
             SubreadAction subreadAction = new SubreadAction();
             subreadAction.setRefer_index_subread(refer_index_workCustom);
             subreadAction.setSubread_type(workflow_type);
             subreadAction.setInput_Subread_reads_single(remote_sequence);
             subreadAction.setInput_Subread_reads_pair1(remote_pair1);
             subreadAction.setInput_Subread_reads_pair2(remote_pair2);
             subreadAction.setInput_subread_reference(remote_re);
             subreadAction.setOptionalParameters_index_Subreadl(1);
             subreadAction.setOptionalParameters_align_Subread(workflowCustom_option_Subread);
             subreadAction.setInput_Subread_path(workflowCustom_path);
             subreadAction.SubreadnoUpload();
             stdoutlist_mapping1 = subreadAction.getStdoutlist();
             //////////////////////////////////////////////////////////////////////////////////////////////////////////
             SubreadAction subreadAction1 = new SubreadAction();
             subreadAction1.setSubread_type(workflow_type);
             subreadAction1.setRefer_index_subread(refer_index_workCustom);
             subreadAction1.setInput_Subread_reads_single(remote_sequence);
             subreadAction1.setInput_subread_reference(remote_re);
             subreadAction1.setOptionalParameters_index_Subreadl(1);
             subreadAction1.setOptionalParameters_align_Subread(workflowCustom_option_Subread);
             subreadAction1.setInput_Subread_path(workflowCustom_path);
             subreadAction1.SubreadnoUpload();
             stdoutlist_mapping2 = subreadAction1.getStdoutlist();
         }
         /************************************************************************************************/
         else {
             NGMAction ngmAction = new NGMAction();
             ngmAction.setRefer_index_NGM(refer_index_workCustom);
             ngmAction.setNGM_type(workflow_type);
             ngmAction.setInput_NGM_reads(remote_sequence);
             ngmAction.setInput_NGM_reads_pair1(remote_pair1);
             ngmAction.setInput_NGM_reads_pair2(remote_pair2);
             ngmAction.setInput_NGM_reference(remote_re);
             ngmAction.setOptionalParameters_NGM(workflowCustom_option_NGM);
             ngmAction.setInput_NGM_path(workflowCustom_path);
             ngmAction.NGM_noUpload();
             stdoutlist_mapping1 = ngmAction.getStdoutlist();
             //////////////////////////////////////////////////////////////////////////////////////////////////////////
             NGMAction ngmAction1 = new NGMAction();
             ngmAction1.setNGM_type(workflow_type);
             ngmAction1.setRefer_index_NGM(refer_index_workCustom);
             ngmAction1.setInput_NGM_reads(remote_control);
             ngmAction1.setInput_NGM_reference(remote_re);
             ngmAction1.setOptionalParameters_NGM(workflowCustom_option_NGM);
             ngmAction1.setInput_NGM_path(workflowCustom_path);
             ngmAction1.NGM_noUpload();
             stdoutlist_mapping2 = ngmAction1.getStdoutlist();
         }
/***************************************Peakcalling***************************************************************/
         if (workflowCustom_peak == 1) {
             MACS_NEWAction macsAction = new MACS_NEWAction();
             macsAction.setInput_controlfile(workflowCustom_path + "/" + control + ".sam");
             macsAction.setInput_tagfile(workflowCustom_path + "/" + original + ".sam");
             macsAction.setOptionalParameters(workflowCustom_option_macs);
             macsAction.setMacs_gs(macs_gs);
             macsAction.setMacs_p(macs_p);
             macsAction.setMacs_bw(macs_bw);
             macsAction.setMacs_s(macs_s);
             macsAction.setInput_macs_path(workflowCustom_path);
             macsAction.MACS_noUpload();
             stdoutlist_peak = macsAction.getStdoutlist();
         } else if (workflowCustom_peak == 2) {
             MACS2_NEWAction macs2Action = new MACS2_NEWAction();
             macs2Action.setInput_tagfile_macs2(workflowCustom_path + "/" + original + ".sam");
             macs2Action.setInput_controlfile_macs2(workflowCustom_path + "/" + control + ".sam");
             macs2Action.setInput_macs_path_macs2(workflowCustom_path);
             macs2Action.setOptionalParameters_macs2(workflowCustom_option_macs2);
             macs2Action.setMacs2_format(2);
             macs2Action.MACS2noUpload();
             stdoutlist_peak = macs2Action.getStdoutlist();
         } else if (workflowCustom_peak == 3) {
             PeakSeqAction peakSeqAction = new PeakSeqAction();
             peakSeqAction.setInput_tagfile_peakseq(workflowCustom_path + "/" + original + ".sam");
             peakSeqAction.setInput_controlfile_peakseq(workflowCustom_path + "/" + control + ".sam");
             peakSeqAction.setInput_path_peakseq(workflowCustom_path);
             peakSeqAction.PeakseqnoUpload();
             stdoutlist_peak = peakSeqAction.getStdoutlist();
         } else if (workflowCustom_peak == 4) {
             PeakRangerAction peakRangerAction = new PeakRangerAction();
             peakRangerAction.setInput_tagfile_peakranger(workflowCustom_path + "/" + original + ".sam");
             peakRangerAction.setInput_controlfile_peakranger(workflowCustom_path + "/" + control + ".sam");
             peakRangerAction.setInput_path_peakranger(workflowCustom_path);
             peakRangerAction.setPeakranger_format(1);
             peakRangerAction.setPeakranger_annofile(peakranger_annofile);
             peakRangerAction.setPeakranger_annofile1(peakranger_annofile1);
             peakRangerAction.setPeakranger_annofile2(peakranger_annofile2);
             peakRangerAction.setPeakranger_length(peakranger_length);
             peakRangerAction.setPeakranger_length1(peakranger_length1);
             peakRangerAction.setPeakranger_length2(peakranger_length2);
             peakRangerAction.setPeakranger_winsize1(peakranger_winsize1);
             peakRangerAction.setPeakranger_winsize2(peakranger_winsize2);
             peakRangerAction.setPeakranger_winstep1(peakranger_winstep1);
             peakRangerAction.setPeakranger_fdr(peakranger_fdr);
             peakRangerAction.setPeakranger_fdr1(peakranger_fdr1);
             peakRangerAction.setPeakranger_imcount1(peakranger_imcount1);
             peakRangerAction.setPeakranger_minscore(peakranger_minscore1);
             peakRangerAction.setPeakranger_relength(peakranger_relength);
             peakRangerAction.setPeakranger_relength1(peakranger_relength1);
             peakRangerAction.setPeakranger_relength2(peakranger_relength2);
             peakRangerAction.setPeakranger_pvalue(peakranger_pvalue);
             peakRangerAction.setPeakranger_pvalue1(peakranger_pvalue1);
             peakRangerAction.setPeakranger_delta(peakranger_delta);
             peakRangerAction.setPeakranger_bandwidth(peakranger_bandwidth);
             peakRangerAction.setPeakranger_pad(peakranger_pad);
             peakRangerAction.setPeakranger_t(peakranger_t);

             if (workflowCustom_option_peakranger == 1) {
                 peakRangerAction.setSelect_peakranger(1);
             } else if (workflowCustom_option_peakranger == 2) {
                 peakRangerAction.setSelect_peakranger(2);
             } else {
                 peakRangerAction.setSelect_peakranger(3);
             }
             peakRangerAction.PeakrangernoUpload();
             stdoutlist_peak = peakRangerAction.getStdoutlist();
         } else if (workflowCustom_peak == 5 || workflowCustom_peak == 6) {
             SICERAction sicerAction = new SICERAction();
             sicerAction.setInput_sicer_tagfile(workflowCustom_path + "/" + original + ".sam");
             sicerAction.setInput_sicer_controlfile(workflowCustom_path + "/" + control + ".sam");
             sicerAction.setInput_sicer_path(workflowCustom_path);
             sicerAction.setOptionalParameters_sicer(workflowCustom_option_sicer);
             sicerAction.SICERnoUpload();
             stdoutlist_peak = sicerAction.getStdoutlist();
         }
        /*else if(workflowCustom_peak==6){
         PePrAction pePrAction=new PePrAction();
         pePrAction.setInput_pepr_tagfile1();
     }*/
         else if (workflowCustom_peak == 7) {
             BCPAction bcpAction = new BCPAction();
             bcpAction.setInput_BCP_tagfile(workflowCustom_path + "/" + original + ".sam");
             bcpAction.setInput_BCP_controlfile(workflowCustom_path + "/" + control + ".sam");
             bcpAction.setInput_BCP_path(workflowCustom_path);
             bcpAction.setBCP_type(2);
             bcpAction.setOptionalParameters_BCP(workflowCustom_option_BCP);
             bcpAction.BCPnoUpload();
             stdoutlist_peak = bcpAction.getStdoutlist();
         } else if (workflowCustom_peak == 8) {
             MACS_NEWAction macsAction = new MACS_NEWAction();
             macsAction.setInput_controlfile(workflowCustom_path + "/" + control + ".sam");
             macsAction.setInput_tagfile(workflowCustom_path + "/" + original + ".sam");
             macsAction.setOptionalParameters(workflowCustom_option_macs);
             ...
             macsAction.MACS_noUpload();
             stdoutlist_peak = macsAction.getStdoutlist();
         } else if (workflowCustom_peak == 9) {
             SISSRAction sissrAction = new SISSRAction();
             sissrAction.setInput_SISSR_tagfile(workflowCustom_path + "/" + original + ".sam");
             sissrAction.setInput_SISSR_controlfile(workflowCustom_path + "/" + control + ".sam");
             sissrAction.setInput_SISSR_path(workflowCustom_path);
             sissrAction.setOptionalParameters_SISSR(workflowCustom_option_SISSRs);
             sissrAction.setInput_SISSR_size("3080000000");
             sissrAction.SISSRnoUpload();
             stdoutlist_peak = sissrAction.getStdoutlist();
         }
        /*else if(workflowCustom_peak==10){
         FindPeaksAction findPeaksAction=new FindPeaksAction();
         findPeaksAction.set
     }*/
         else if (workflowCustom_peak == 11 || workflowCustom_peak == 10) {
             AREMAction aremAction = new AREMAction();
             aremAction.setInput_AREM_file(workflowCustom_path + "/" + original + ".sam");
             aremAction.setInput_AREM_control(workflowCustom_path + "/" + control + ".sam");
             aremAction.setInput_AREM_path(workflowCustom_path);
             aremAction.setOptionalParameters_AREM(workflowCustom_option_AREM);
             aremAction.AREMnoUpload();
             stdoutlist_peak = aremAction.getStdoutlist();
         } else if (workflowCustom_peak == 12) {
             FseqAction fseqAction = new FseqAction();
             fseqAction.setInput_Fseq_file(workflowCustom_path + "/" + original + ".sam");
             fseqAction.setInput_Fseq_control(workflowCustom_path + "/" + control + ".sam");
             fseqAction.setInput_Fseq_path(workflowCustom_path);
             fseqAction.setOptionalParameters_Fseq(workflowCustom_option_fseq);
             fseqAction.Fseq_exe();
         } else {
             MACS_NEWAction macsAction = new MACS_NEWAction();
             macsAction.setInput_controlfile(workflowCustom_path + "/" + control + ".sam");
         ...
         ///////////////////////////////////////////////////////////////////////////////////////
         IGV_NEWAction igvAction = new IGV_NEWAction();
         igvAction.setMapping_radio(1);
         igvAction.setSampath(sampath);
         igvAction.setWorkflowCustom_reference(remote_re);
         workflowCustom_reference = remote_re;
         igvAction.setWorkflowCustom_path(workflowCustom_path);
         igvAction.setMapping_visual_scope("all");
         igvAction.visual_exe();

         IGV_PeakAction igv_peakAction = new IGV_PeakAction();
         igv_peakAction.setReference(remote_re);
         igv_peakAction.setTreatment(bampath_treatment);
         igv_peakAction.setControl(bampath_control);
         igv_peakAction.setPeak(peakpath);
         igv_peakAction.setScope("all");
         igv_peakAction.setPath(workflowCustom_path);
         igv_peakAction.IGV_Peak();
         flag_command="1";
         ...
         File file1 = new File(bampath_control);
         String name1 = file1.getName();
         File file2 = new File(remote_re);
         String name2 = file2.getName();
         xmlUtil.xmlAdd_Jobs(JobId,name + " ; " + name1 + " ; " + name2, "WorkFlow", stdoutlist_mapping1+stdoutlist_mapping2+stdoutlist_peak,"email",time);
    ...
    public void validate(){
        ...
        if(workflow_type!=1&&workflow_type!=2){
            this.addFieldError("workflow_type", "Please choose the type of input.");
        }
        if(workflow_type==1){
            if(workflowCustom_sequence.equals("")){
                this.addFieldError("workflowCustom_sequence", "Please choose the sequence.");
            }
        }
        else{
            if(workflowCustom_sequence_pair1text.equals("")){
                this.addFieldError("workflowCustom_sequence_pair1text", "Please input the sequence.");
            }
        }
        if(refer_index_workCustom==1){
            if(workflowCustom_reference.equals("")){
                this.addFieldError("workflowCustom_reference", "Please input the reference.");
            }
        }
      if(workflowCustom_control.equals("")){
          this.addFieldError("workflowCustom_control", "Please input the conrol file.");
      }
      if(workflowCustom_mapping!=1&&workflowCustom_mapping!=2&&workflowCustom_mapping!=3&&workflowCustom_mapping!=4&&workflowCustom_mapping!=5&&workflowCustom_mapping!=6&&workflowCustom_mapping!=7){
          this.addFieldError("workflowCustom_mapping", "Please choose the mapping tool.");
      }
      if(workflowCustom_peak<1||workflowCustom_peak>13){
            this.addFieldError("workflowCustom_peak", "Please choose the peak calling tool.");
        }
       /* xmlUtil xmlUtil=new xmlUtil();
